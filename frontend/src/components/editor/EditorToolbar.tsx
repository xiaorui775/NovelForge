...
export default function EditorToolbar({
  ...
  onGenerate, onContinue, onRefine,
}: EditorToolbarProps) {
  return (
    <div className="space-y-4">
      ...
      {hasContent && onTogglePolishingMode && (
        <button onClick={onTogglePolishingMode} className="btn-ghost w-full flex items-center justify-center gap-2 text-sm border border-ink/20">
      ...
      {hasContent && onBrainstorm && (
        <button
          ...
          className="btn-ghost w-full flex items-center justify-center gap-2 text-sm border border-amber-500/30 text-amber-300 hover:text-amber-200"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
      ...
      <div className="space-y-2">
        <button onClick={onGenerate} disabled={generating || !selectedModel} className="btn-primary w-full flex items-center justify-center gap-2 text-sm">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
          ...
          <kbd className="ml-auto text-[10px] text-parchment/40 font-mono">⌘G</kbd>
        </button>

        {hasContent && (
          <button onClick={onContinue} disabled={generating || !selectedModel} className="btn-secondary w-full flex items-center justify-center gap-2 text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
        ...
        {hasContent && onRefine && (
          <button onClick={onRefine} disabled={generating || !selectedModel} className="btn-ghost w-full flex items-center justify-center gap-2 text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.93z" />
              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 7.125L16.875 4.5" />
            </svg>
